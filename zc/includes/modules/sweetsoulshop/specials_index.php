...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' =>' data-mp3="res/univ.mp3" data-title="' . $featured_products->fields['products_name'] . ' / the Brand New Heavies" data-price="' . $data_price . '"',
    'text' => '<div class="thumb mask">' . zen_image(DIR_WS_IMAGES . $specials_index->fields['products_image'], $specials_index->fields['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '<span class="mask-hover player-play"><p><a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'products_id=' . (int)$specials_index->fields['products_id']) . '">&gt;&nbsp;See Detail</a></p></span></div><p class="desc flag flag-deu"><a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'products_id=' . (int)$specials_index->fields['products_id']) . '"><strong>' . $specials_index->fields['products_name'] . '</strong><br /><span class="caption">' . $specials_index->fields['products_name'] . '</span></a><br /><a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'products_id=' . (int)$specials_index->fields['products_id']) . '" class="btn-buy">' . $products_price . '</a></p>');
