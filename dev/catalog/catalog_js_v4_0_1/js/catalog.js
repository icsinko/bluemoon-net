...
(function($){
	...
	$.fn.catalogSet = catalogSet = function() {
		...
		var settings = {
			...
			viewIndex : 'no',             //indexページ表示 yes or no
	...
	catalogSet.setParam = {
		...
		create: function(settings,elem) {
			...
			this.co.menu = this.element.find('#menu'); //#menu
			this.co.menuBtn = this.element.find('#menuBtn'); //#menu
			this.co.menuList = this.element.find('#menuList'); //#menu
			this.co.menOpen = false; //#menu
		...
		setSlide: function() {
			...
			self.co.slide.each(function(i){
				...
				if(self.co.ieVer == 'ie9' || self.co.ieVer == 'noie'){
					// ie以外もしくはie9
					var imgCls = my.find("img").attr("class");
					var imgClsi = imgCls.substr(-3);
					imgclass = '.' + imgClsi;
				} else {
				...
				catalogImg.onload = function() {
					...
					// link
					self.co.links[i] = my.find(".slideLink a").attr("href");
					...
					if(count == self.co.slide.length) {
						...
						//storeLink
						var pageLink = self.co.links[0];
						if(pageLink){
							$(".storeBtn a").attr("href",pageLink);
							$(".storeBtn").show();
						} else {
							$(".storeBtn").hide();
						}
...
*/
		...
		setMenu: function() {
			...
			$("#menuBtn").click(function(e) {
				if(self.co.menuOpen){
					self.co.menuList.animate({
						right:"-160px"
					},500);
					self.co.menu.css("right","-100%");
					self.co.menuOpen = false;
				} else {
					self.co.menu.css("right",0);
					self.co.menuList.animate({
						right:0
					},500);
					self.co.menuOpen = true;
				}
				return false;
		...
		checkPage: function() {
			...
			// storeLink
			var pageLink = self.co.links[self.co.currentPage-1];
			if(pageLink){
				$(".storeBtn a").attr("href",pageLink);
				$(".storeBtn").show();
			} else {
