...
define('HEADER_TITLE_SUPPORT_SITE_JP', 'Support Site For Japan');
...
// admin activity log warnings
  ...
  define('WARNING_REGISTER_GLOBALS_ON', 'As for the server being used, register_globals is operated with On.<br/>Zen Cart doesn\'t operate if the setting of register_globals is not set Off because register_globals has the possibility of causing a serious problem on security.<br/>The setting of register_globals cannot be changed with .htaccess in the server when this message is displayed.<br/>There are two solution to solve this problem.<br/><br/><br/>1. Change the setting of the server.<br/>2. Reinstall Zen Cart to the server which is able to be set register_globals Off.');
